...
if __name__ == '__main__':
    # 加载模型
    model = YOLO(r'D:\Projects\pythonProject\YOLO11\ultralytics\cfg\models\v8\yolov8.yaml').load(r'D:\Projects\pythonProject\YOLO11\yolov8n.pt')
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data=r"E:\DSA_AI\兔子+猪\检测\my.yaml",
        epochs=250,  # (int) 训练的周期数
        patience=150,  # (int) 等待无明显改善以进行早期停止的周期数
        batch=8,  # (int) 每批次的图像数量（-1 为自动批处理）
        imgsz=512,  # (int) 输入图像的大小，整数或w，h
        ...
        project='runs/test_xiong',  # (str, optional) 项目名称
        name='20250820/猪+兔子',  # (str, optional) 实验名称，结果保存在'project/name'目录下
